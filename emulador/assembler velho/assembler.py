...
def is_label(string):                # Informa se uma string é uma label.
    ...
    return (not(string in no_operand_dict.keys())) and (not(string in operand_dict.keys()))
...
def generate_file(): # Gera o arquivo final.
    ...
    bytes_to_be_added[16] = (0x1001 + next_constant) & 0xff
    bytes_to_be_added[17] = ((0x1001 + next_constant)>>8) & 0xff
    bytes_to_be_added[18] = ((0x1001 + next_constant)>>16) & 0xff
    bytes_to_be_added[19] = ((0x1001 + next_constant)>>24) & 0xff
    ...
    for byte in byte_list:                                         # Escreve os bytes das instruções, labels e operandos.
        try:
            ...
            print("Erro 404: not label found")
